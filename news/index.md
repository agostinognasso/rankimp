...
- [`importance_judges()`](../reference/importance_judges.md) builds the
  panel from fitted models along the four axes of the roadmap — method,
  model, seed, resample — and records the provenance, raw scores and
  per-judge weights on the returned `judges` object, which
...
- `rank_confsets(type = "data")` resamples the *rows* instead of the
  ...
  judge bootstrap cannot — whether the ranking would survive another
  sample — and it needs a panel from
  ...
  Replicates that fail — a response class too rare to survive a draw is
  the case that happens — are dropped, counted and warned about instead
  of killing the run.
...
- [`autoplot()`](https://ggplot2.tidyverse.org/reference/autoplot.html)
  ...
  that was split, and this is the difference — per variable, where
...
- [`vignette("credit-scoring")`](../articles/credit-scoring.md) runs the
  four axes at once on a synthetic portfolio — two model families, two
  methods, three folds, two seeds, 24 judges — and is no longer a
  placeholder. Its point is what the package refuses to say: the three
...
- [`consensus_rank()`](../reference/consensus_rank.md) no longer reports
  ...
  case. The optima are now combined — each variable takes its average
  position over them and those that come out equal are tied — so
...
- [`prob_topk()`](../reference/prob_topk.md) and
  ...
  conservative in the middle of its range and accurate at the ends — a
  variable given 0.44 is in the top `k` 56% of the time, one given 0.98
  is there 98% of the time — and
...
- The warning in [`?rank_confsets`](../reference/rank_confsets.md) that
  ...
  and the bottom drifts up — 0.55 ranks for the second variable, 1.00
  for the eighth.
...
- [`importance_judges()`](../reference/importance_judges.md) records the
  recipe that built the panel — the fits, the data, the target, the
  methods, the axes and the backend settings — as an attribute of the
