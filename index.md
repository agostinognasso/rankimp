...
while permutation importance puts it first. See
[`?applications`](reference/applications.md).

[`vignette("credit-scoring")`](articles/credit-scoring.md) is the worked
example on it, and it is worth reading for the result: the consensus of
24 judges gets the top of the ordering wrong, and the confidence sets
are narrow and wrong along with it, because half the panel shares a bias
...
produce. [`importance_to_rank()`](reference/importance_to_rank.md)
converts scores to ranks, and gives equal scores equal ranks instead of
separating them by column order.
...
works through a case where two methods differ by a factor of six in the
scores and produce the same ordering.

### A panel instead of a ranking

[`importance_judges()`](reference/importance_judges.md) builds the
panel. It runs the requested backends over every combination of four
axes, methods, models, seeds and resamples, and returns a `judges`
object: an integer matrix with one row per judge, plus the provenance
and the raw scores.
...
[`consensus_rank()`](reference/consensus_rank.md) returns the ranking
with the smallest total Kemeny-Snell distance to all the judges.
...
![](reference/figures/README-plot_consensus-1.png)
...
[`item_consensus()`](reference/item_consensus.md) scores each judge
against the reported consensus, worst first.
...
[`judge_weights()`](reference/judge_weights.md) turns that into votes.
`by = "method"` lets you hand impurity half a vote because you do not
trust it here; `by = "reliability"` gives each judge a weight that grows
with its agreement with the rest of the panel.
...
spread evenly or whether the panel has a seam in it.
[`judge_clusters()`](reference/judge_clusters.md) looks for the seam by
k-medians in Kemeny-Snell space, where each group’s centre is the Kemeny
median of its own members and is therefore a ranking you can report.
...
[`rank_confsets()`](reference/rank_confsets.md) puts an interval around
each variable’s rank. There are two bootstraps and they answer different
questions.
...
![](reference/figures/README-plot_confsets-1.png)
...
[`prob_topk()`](reference/prob_topk.md) reports how often each variable
landed in the top `k` across the replicates.
...
[`rank_select()`](reference/rank_select.md) keeps the variables whose
entire interval clears a threshold. This is the function to reach for
when someone is going to act on the answer.
...
[`vignette("theory")`](articles/theory.md) is the full argument. The
short version:
...
$`d_{KS}`$ stop being the same problem. $`\tau_x`$ restores it, which
makes the consensus and the agreement statistic two views of one
optimisation rather than two numbers that sit near each other.
...
|----|----|
| How important does this one method say each variable is? | [`importance_permutation()`](reference/importance_backends.md), [`importance_mdi()`](reference/importance_backends.md), [`importance_loco()`](reference/importance_backends.md), [`importance_shap()`](reference/importance_backends.md) |
| I have scores; give me a ranking with honest ties | [`importance_to_rank()`](reference/importance_to_rank.md) |
...
| Show me the panel, the intervals, the groups | [`autoplot()`](https://ggplot2.tidyverse.org/reference/autoplot.html) |
...
| [`prob_topk()`](reference/prob_topk.md) understates rather than overstates | 0.44 reported against 56% actual; 0.98 against 98% |
| [`rank_select()`](reference/rank_select.md) is conservative | at most 3% undeserved selections, and none at the strongest thresholds |
| [`judge_clusters()`](reference/judge_clusters.md) refuses fake groups | silhouette alone splits one population 62% of the time; the calibrated test does not |
...
share the bias.
[`vignette("credit-scoring")`](articles/credit-scoring.md) shows the
same mechanism doing real damage. Had every judge been an impurity
judge, the consensus would have been narrow, confident and wrong in the
...
|----|----|
| [`vignette("rankimp-intro")`](articles/rankimp-intro.md) | the short tour: panels, the consensus, ties, weights |
| [`vignette("reference")`](articles/reference.md) | every function, what it returns, and when you want it |
| [`vignette("theory")`](articles/theory.md) | why a Kemeny median, and what a bootstrapped rank estimates |
| [`vignette("stability")`](articles/stability.md) | rank confidence sets and what they were measured to cover |
| [`vignette("method-disagreement")`](articles/method-disagreement.md) | panels that split, and reading one that does not |
| [`vignette("credit-scoring")`](articles/credit-scoring.md) | all four axes end to end on one problem |
| [`vignette("against-set-stability")`](articles/against-set-stability.md) | how this differs from `stabm` |
...
`ConsRank`; [`rank_confsets()`](reference/rank_confsets.md) orchestrates
nothing, and it is the part that lets a claim about variable importance
be falsified.
